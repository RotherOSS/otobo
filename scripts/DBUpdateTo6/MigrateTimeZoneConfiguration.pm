...
sub CheckPreviousRequirement {
    ...
    if ( $OTOBOTimeZone && $UserDefaultTimeZone ) {
        return 1;
    }
    ...
    # Calculate complete time offset (server time zone + OTOBO time offset).
    ...
    if ( exists $TimeZoneByOffset->{$TimeOffset} ) {
        print
            "\n\n        The currently configured time offset is $TimeOffset hours, these are the suggestions for a corresponding OTOBO time zone: \n\n";

        print "        " . join( "\n        ", sort @{ $TimeZoneByOffset->{$TimeOffset} } ) . "\n";
    }

    if ( $SuggestedTimeZone && $TimeZones{$SuggestedTimeZone} ) {
        print "\n\n        It seems that $SuggestedTimeZone should be the correct time zone to set for your OTOBO. \n";
    }

    $Self->{TargetTimeZones}->{OTOBOTimeZone} = $Self->_AskForTimeZone(
        ConfigKey => 'OTOBOTimeZone',
        TimeZones => \%TimeZones,
    );
...
sub _CheckForTimeZones {
    ...
    my $OTOBOTimeOffset  = int( $ConfigObject->Get('TimeZone') // 0 ) + $SystemOffset;
    my %TimeZone2Offset = (
        OTOBOTimeZone        => $OTOBOTimeOffset,
        UserDefaultTimeZone => $OTOBOTimeOffset,
    );

    CALENDAR:
    for my $Calendar ( 1 .. 9 ) {
        my $ConfigKey        = "TimeZone::Calendar$Calendar";
        my $CalendarTimeZone = int( $ConfigObject->Get($ConfigKey) // 0 ) + $OTOBOTimeOffset;

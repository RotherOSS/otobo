...
sub Run {
    ...
    if (
        $OTOBO5Config{'Ticket::StorageModule'}
        && $OTOBO5Config{'Ticket::StorageModule'} eq 'Kernel::System::Ticket::ArticleStorageFS'
        )
    {
    ...
    if ( $OTOBO5Config{'Ticket::StorageModule::CheckAllBackends'} ) {
        my $Result = $Self->SettingUpdate(
    ...
    if ( $OTOBO5Config{'ArticleDir'} ) {
        my $Result = $Self->SettingUpdate(
            Name           => 'Ticket::Article::Backend::MIMEBase::ArticleDataDir',
            IsValid        => 1,
            EffectiveValue => $OTOBO5Config{'ArticleDir'},
            UserID         => 1,
        );

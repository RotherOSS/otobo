...
;
-- ----------------------------------------------------------
--  create table article_data_otobo_chat
-- ----------------------------------------------------------
CREATE TABLE article_data_otobo_chat (
    id NUMBER (20, 0) NOT NULL,
    article_id NUMBER (20, 0) NOT NULL,
...
ALTER TABLE article_data_otobo_chat ADD CONSTRAINT PK_article_data_otobo_chat PRIMARY KEY (id);
BEGIN
    EXECUTE IMMEDIATE 'DROP SEQUENCE SE_article_data_otobo_chat';
EXCEPTION
    WHEN OTHERS THEN NULL;
END;
/
--
;
CREATE SEQUENCE SE_article_data_otobo_chat
INCREMENT BY 1
START WITH 1
NOMAXVALUE
NOCYCLE
...
;
BEGIN
    EXECUTE IMMEDIATE 'DROP TRIGGER SE_article_data_otobo_chat_t';
EXCEPTION
    WHEN OTHERS THEN NULL;
END;
/
--
;
CREATE OR REPLACE TRIGGER SE_article_data_otobo_chat_t
BEFORE INSERT ON article_data_otobo_chat
FOR EACH ROW
BEGIN
    IF :new.id IS NULL THEN
        SELECT SE_article_data_otobo_chat.nextval
        INTO :new.id
        FROM DUAL;
    END IF;
END;
...
-- ----------------------------------------------------------
CREATE TABLE service_customer_user (
    customer_user_login VARCHAR2 (191) NOT NULL,
    service_id NUMBER (12, 0) NOT NULL,
    create_time DATE NOT NULL,
    create_by NUMBER (12, 0) NOT NULL
);

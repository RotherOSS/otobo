...
my @DatabaseXMLFiles = (
    "$Home/scripts/test/sample/DBUpdate/otobo5-schema.xml",
    "$Home/scripts/test/sample/DBUpdate/otobo5-initial_insert.xml",
);
...
my $XMLString = $Kernel::OM->Get('Kernel::System::Main')->FileRead(
    Location => "$Home/scripts/test/sample/DBUpdate/otobo5-initial_insert_chat.xml",
);
...
my @NewTablesName = ( 'article_data_mime', 'article_data_otobo_chat' );
...
my $CheckData = sub {
    ...
    if ( $Test eq 'Is' ) {
        ...
        $Self->IsNot(
            $ArticleChatCount,
            $ChatCount,
            "The same amount of rows should NOT be present in article_data_mime($ArticleChatCount) and article_data_otobo_chat($ChatCount) tables.",
        );
        $Self->True(
            1,
            'The article_data_mime and article_data_otobo_chat tables have not chat rows.',
        );
    }
    else {
        $Self->IsNot(
            $ArticleChatCount,
            $ChatCount,
            'In article_data_mime and article_data_otobo_chat should not be the same amount of data.',
        );
        $Self->IsNot(
            $ArticleChatCount,
            0,

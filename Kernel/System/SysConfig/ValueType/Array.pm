...
package Kernel::System::SysConfig::ValueType::Array;
...
Kernel::System::SysConfig::ValueType::Array - System configuration array value type backed.
...
Create an object. Do not use it directly, instead use:
    ...
    my $ValueTypeObject = $Kernel::OM->Get('Kernel::System::SysConfig::ValueType::Array');
...
sub SettingEffectiveValueCheck {
    ...
    # Data should be array ref.
    if ( !IsArrayRefWithData($Param{EffectiveValue}) ) {
        $Result{Error} = 'EffectiveValue must be an array ref!';
        return %Result;
    }
...
sub AddItem {
    ...
    if ( $Param{DefaultItem} && $Param{DefaultItem}->{Item} ) {
        $DefaultValue = $Param{DefaultItem} && $Param{DefaultItem}->{Item} || [];
    }
    elsif ( $Param{DefaultItem} ) {
        $DefaultValue = $Param{DefaultItem} || [];
    }

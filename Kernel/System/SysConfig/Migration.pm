...
sub MigrateXMLStructure {
    ...
    for my $Setting (@Settings) {
        ...
        {
            $Setting
                =~ s{<Item Key="X-OTOBO-ArticleType">(.*?)</Item>}{<Item Key="X-OTOBO-IsVisibleForCustomer">$ArticleTypeMapping{$1}->{Visible}</Item>}g;
            $Setting
                =~ s{<Item Key="X-OTOBO-FollowUp-ArticleType">(.*?)</Item>}{<Item Key="X-OTOBO-FollowUp-IsVisibleForCustomer">$ArticleTypeMapping{$1}->{Visible}</Item>}g;
            $Setting
    ...
    $Result =~ s{</otobo_config>.*}{</otobo_config>}gsmx;

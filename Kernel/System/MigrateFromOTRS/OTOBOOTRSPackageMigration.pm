...
package Kernel::System::MigrateFromOTRS::OTOBOOTRSPackageMigration;    ## no critic
...
Kernel::System::MigrateFromOTRS::OTOBOOTRSPackageMigration - Check which packages are installed on both systems
...
sub Run {
    ...
    for my $Key (qw(DBData)) {
        if ( !$Param{$Key} ) {
            ...
            $Result{Message}    = $Self->{LanguageObject}->Translate( 'Check if %s!', $Key );
            $Result{Successful} = 0;
            return \%Result;
        }
    }
    ...
    for my $Key (qw(DBDSN DBType DBHost DBUser DBPassword DBName)) {
        if ( !$Param{DBData}->{$Key} ) {
            ...
            $Result{Comment}    = $Self->{LanguageObject}->Translate( 'Check if %s!', $Key );
            $Result{Successful} = 0;
            return \%Result;
        }
    }
    ...
    $CacheObject->Set(
        ...
        Key   => 'MigrationState',
        Value => {
            Task        => 'OTOBOOTRSPackageMigration',
            SubTask     => 'Rebuild OTRS Packages for OTOBO.',
            StartTime   => $Epoch,
        },
    );
    ...
    if ( IsArrayRefWithData(\@OTRSPackages) ) {
            ...
            $Result{Comment}    = $Self->{LanguageObject}->Translate( "The following packages are only installed in OTRS:" ) . $MessageString
                                .$Self->{LanguageObject}->Translate( "Please install (or uninstall) the packages before migration. If a package doesn't exist for OTOBO so far, please contact the OTOBO Team at bugs\@otobo.org. We will find a solution." );
            $Result{Successful} = 0;
